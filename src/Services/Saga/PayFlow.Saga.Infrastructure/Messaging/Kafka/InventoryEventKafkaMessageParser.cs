...
public static class InventoryEventKafkaMessageParser
{
    public const string Topic = "inventory.events";
    public const string InventoryReservedMessageType =
    ...
    public static ConsumedInventoryReservedMessage ParseReserved(
        ...
        DateTimeOffset receivedAtUtc)
    {
        var parsed = ParseEnvelope(
            consumeResult,
            receivedAtUtc,
            InventoryReservedMessageType);

        var payload =
            Deserialize<InventoryReservedV1>(
        ...
        return new ConsumedInventoryReservedMessage(
    ...
    public static ConsumedInventoryReservationRejectedMessage ParseRejected(
        ConsumeResult<string, string> consumeResult,
        DateTimeOffset receivedAtUtc)
    {
        var parsed = ParseEnvelope(
            consumeResult,
            receivedAtUtc,
            InventoryRejectedMessageType);

        var payload =
            Deserialize<InventoryReservationRejectedV1>(
                consumeResult.Message.Value,
                InventoryRejectedMessageType);
        ...
        string messageType)
    {
        try
        {
            return JsonSerializer.Deserialize<T>(
                    ...
                    SerializerOptions)
                ?? throw new InvalidDataException(
                    $"{messageType} payload deserialized to null.");
        }
        catch (JsonException exception)
        {
            throw new InvalidDataException(
                $"Kafka payload is not valid {messageType} JSON.",
                exception);
        }
    }

    private static void EnsureOrderId(
        Guid payloadOrderId,
        Guid aggregateId)
    {
        if (payloadOrderId == Guid.Empty
            || payloadOrderId != aggregateId)
        {
            throw new InvalidDataException(
